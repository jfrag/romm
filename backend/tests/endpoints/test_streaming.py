...
def test_slot_from_state_filename():
    ...
    assert states.slot_from_state_filename("scummvm", "woodruff-win-fr.s01") == 1
    assert states.slot_from_state_filename("scummvm", "monkey.001") == 1
    assert states.slot_from_state_filename("scummvm", "monkey.s00") is None
    assert states.slot_from_state_filename("scummvm", "monkey.sav") is None
...
def test_stamped_state_filename_round_trips_for_scummvm():
    ...
    when = datetime(2026, 7, 21, 4, 56, 45, 123456, tzinfo=timezone.utc)
    stamped = states.stamped_state_filename("scummvm", "woodruff-win-fr.s01", when)
    assert re.fullmatch(r"woodruff-win-fr\.\d{8}-\d{12}\.s01", stamped)
    assert states.container_state_filename(stamped) == "woodruff-win-fr.s01"
    assert states.slot_from_state_filename("scummvm", stamped) == 1
